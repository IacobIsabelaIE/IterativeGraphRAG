...
def extract_initial_entities(question: str, rag, top_k=50) -> list:
    """
    Combines NER-based and semantic retrieval-based entity discovery.
    Uses spaCy NER to find named entities in the question,
    and also retrieves semantically related entities via LanceDB.
    """
    doc = nlp(question)
    ner_names = [ent.text for ent in doc.ents]

    initial_ids = set()
    name_to_id = {v.lower(): k for k, v in rag.id2name.items()}

    for name in ner_names:
        lower_name = name.lower()
        if lower_name in name_to_id:
            initial_ids.add(name_to_id[lower_name])

    print(f"  Found {len(initial_ids)} entities via NER and graph map.")

    print("  Also retrieving semantically related entities from LanceDB...")
    ...
    print(f"  Total candidate entities found (NER + retrieval): {len(initial_ids)}")
